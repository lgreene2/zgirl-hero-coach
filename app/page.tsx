...
const VIDEO_SCRIPT_COUNT_KEY = "zgirl-video-script-count-v1";
const PLAN_KEY = "zgirl-plan-v1"; // "free" | "plus"
...
export default function Home() {
  ...
  // Monetization state (privacy-first: no content is sent for billing/analytics)
  const [isPlus, setIsPlus] = useState(false);
  const [showPaywall, setShowPaywall] = useState(false);
  const [paywallReason, setPaywallReason] = useState<string | null>(null);
  const [videoScriptCount, setVideoScriptCount] = useState(0);
...
useEffect(() => {
  ...
  // Load billing status + limits on mount
  useEffect(() => {
    if (typeof window === "undefined") return;

    // Privacy-safe app open event
    track("app_open", { surface: "home" });

    // Video script counter
    try {
      const n = Number(
        window.localStorage.getItem(VIDEO_SCRIPT_COUNT_KEY) || "0"
      );
      if (!Number.isNaN(n)) setVideoScriptCount(n);
    } catch {
      // ignore
    }

    // Quick local fallback for plan (useful for dev/testing)
    try {
      const plan = window.localStorage.getItem(PLAN_KEY) || "free";
      if (plan === "plus") setIsPlus(true);
    } catch {
      // ignore
    }

    // Billing status (server cookie) — safe default is FREE
    fetch("/api/billing/status")
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        if (data && typeof data.isPlus === "boolean") setIsPlus(data.isPlus);
      })
      .catch(() => {});
  }, []);
  ...
  const requirePlus = (reason: string) => {
  ...
  const startCheckout = async () => {
    track("upgrade_click", { from: paywallReason || "unknown" });
    try {
      const resp = await fetch("/api/stripe/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: paywallReason || "paywall" }),
      });
      if (!resp.ok) throw new Error("checkout_failed");
      const data = await resp.json();
      if (data?.url) window.location.href = data.url;
    } catch {
      setErrorBanner("Could not start checkout right now. Please try again.");
    }
  };
  ...
  const speakText = (text: string) => {
   ...
   const handleSend = async () => {
    ...
    track("chat_send", { mood: selectedMood ? "selected" : "none" });
  ...
  const handleSaveHeroMoment = () => {
    ...
    // Free tier limit
    if (!isPlus && heroMoments.length >= FEATURES.FREE.heroMomentsLimit) {
      requirePlus("hero_moments");
      return;
    }
    ...
    track("hero_moment_save", { mood: selectedMood ? "selected" : "none" });
  ...
  const handleReplayLastReply = () => {
    ...
    if (!isPlus) {
      requirePlus("audio_replay");
      return;
    }

    // 🔔 Optional reply chime
    ...
    track("audio_replay", { source: "hero_moments" });
  };
  ...
  const handleVideoScript = () => {
    ...
    // Free tier limit
    if (!isPlus && videoScriptCount >= FEATURES.FREE.videoScriptLimit) {
      requirePlus("video_script");
      return;
    }
...
"Unwrap the Hero Within."`;
    ...
    // Count usage (privacy-safe)
    try {
      const nextCount = videoScriptCount + 1;
      setVideoScriptCount(nextCount);
      window.localStorage.setItem(VIDEO_SCRIPT_COUNT_KEY, String(nextCount));
    } catch {
      // ignore
    }
    track("video_script_generate", { mood: selectedMood ? "selected" : "none" });
  ...
  const handleMoodClick = (mood: string) => {
    setSelectedMood((prev) => {
      const next = prev === mood ? null : mood;
      track("mood_select", { set: next ? "yes" : "no" });
      return next;
    });
  };
          ...
          <div className="w-full max-w-4xl rounded-3xl bg-slate-900/80 border border-slate-800 shadow-2xl shadow-cyan-500/10 px-6 py-6 md:px-10 md:py-8">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      track("breathing_start", { source: "mood_chip" });
                    }}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.3fr)] gap-6">
              ...
              <aside className="space-y-4 text-xs">
                ...
                <section className="rounded-2xl border border-emerald-500/40 bg-emerald-500/5 px-3 py-3 space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        type="button"
                        onClick={handleReplayLastReply}
                        className="text-[11px] rounded-full bg-slate-900/90 border border-emerald-500/50 px-3 py-1 font-semibold text-emerald-100 hover:bg-slate-900 hover:border-emerald-400 transition"
                      >
                        {isPlus ? "Replay last reply" : "Replay (Plus)"}
                      </button>
                ...
                {/* Weekly Hero Path (Plus) */}
                <section className="rounded-2xl border border-fuchsia-500/40 bg-fuchsia-500/5 px-3 py-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <h2 className="text-[11px] font-semibold text-fuchsia-200">
                      Weekly Hero Path
                    </h2>
                    {!isPlus && (
                      <span className="text-[10px] rounded-full bg-fuchsia-500/10 border border-fuchsia-400/60 px-2 py-0.5 text-fuchsia-100">
                        Plus
                      </span>
                    )}
                  </div>
                  <p className="text-[11px] text-fuchsia-100/80">
                    A 5-minute guided reflection journey that helps students practice a calming skill, reframe self-talk, and choose one small next step.
                  </p>
                  {isPlus ? (
                    <div className="rounded-xl bg-slate-950/80 border border-fuchsia-500/40 px-3 py-2 text-[11px] text-slate-100 space-y-1">
                      <div><span className="text-fuchsia-200 font-semibold">Step 1:</span> Name the feeling + what triggered it</div>
                      <div><span className="text-fuchsia-200 font-semibold">Step 2:</span> One body reset (breath / grounding)</div>
                      <div><span className="text-fuchsia-200 font-semibold">Step 3:</span> Choose one “hero move” for today</div>
                    </div>
                  ) : (
                    <button
                      type="button"
                      onClick={() => requirePlus("hero_path")}
                      className="text-[11px] rounded-full bg-fuchsia-400/90 px-3 py-1 font-semibold text-slate-950 hover:bg-fuchsia-300 transition"
                    >
                      Unlock Weekly Hero Paths
                    </button>
                  )}
                </section>
              </aside>
