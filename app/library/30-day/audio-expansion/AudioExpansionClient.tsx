...
function label(status?: DayStatus) {
  ...
  if (status.failureClass === "QUOTA_EXHAUSTED") return "QUOTA BLOCKED";
...
function labelClass(value: string) {
  ...
  if (value === "QUOTA BLOCKED") return "text-amber-200";
  if (value === "FAILED") return "text-rose-200";
  return "text-slate-500";
}

export default function AudioExpansionClient() {
  ...
  const refresh = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (!payload.representativeGateUnlocked) {
        setAutoRun(false);
        setMessage("Expansion is locked because the 5-track representative approval gate is not complete.");
        ...
        setAutoRun(false);
        setMessage(`Day ${payload.quotaBlockedDay} is quota-blocked. The factory stopped instead of switching voices or retrying repeatedly.`);
      ...
      setAutoRun(false);
      setMessage(`Status check failed (${error instanceof Error ? error.message : "unknown"}). No new provider request was sent.`);
      return null;
    } finally {
    ...
    try {
      ...
      if (!response.ok || !payload?.ok) {
        if (response.status === 429 || payload?.code === "PROVIDER_QUOTA_EXHAUSTED") {
          setAutoRun(false);
          setMessage(`Provider quota blocked Day ${payload?.day ?? day ?? "—"}. The factory stopped with no fallback voice.`);
          await refresh(true);
          return false;
        }
      ...
      if (payload.complete) {
        ...
        setMessage(`Day ${payload.day} queued. Exact approved v3 baseline recipe only; no Gemini 2.5 fallback.`);
      }
      await refresh(true);
      return true;
    } catch (error) {
      setAutoRun(false);
      setMessage(`Could not queue the next track (${error instanceof Error ? error.message : "unknown"}). The expansion state remains intact.`);
      return false;
    } finally {
      setQueueing(false);
    }
  ...
  useEffect(() => {
    if (timerRef.current) window.clearInterval(timerRef.current);
    timerRef.current = window.setInterval(() => void refresh(true), POLL_MS);
    return () => {
      if (timerRef.current) window.clearInterval(timerRef.current);
      timerRef.current = null;
    };
  ...
  useEffect(() => {
    if (!autoRun || loading || queueing || activeRenderDay || quotaBlockedDay || !summary?.representativeGateUnlocked) return;
    if (expansionReady >= expansionTotal || !nextDay) {
      setAutoRun(false);
      return;
    }
    if (autoTimerRef.current) window.clearTimeout(autoTimerRef.current);
    autoTimerRef.current = window.setTimeout(() => void queue(), 3500);
    return () => {
      if (autoTimerRef.current) window.clearTimeout(autoTimerRef.current);
      autoTimerRef.current = null;
    };
  }, [autoRun, loading, queueing, activeRenderDay, quotaBlockedDay, summary?.representativeGateUnlocked, expansionReady, expansionTotal, nextDay, queue]);
  ...
  const retryBlocked = async () => {
    ...
    if (!day) return;
    if (!window.confirm(`Retry Day ${day} once using Gemini 3.1 + Sulafat only? No fallback model will be used.`)) return;
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-12">
      <section className="rounded-[2rem] border border-[#49d8c2]/25 bg-[#49d8c2]/[.05] p-5 sm:p-7">
        <p className="text-xs font-black uppercase tracking-[.2em] text-[#76ead6]">25-track expansion factory · representative gate passed</p>
        <div className="mt-3 grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <h2 className="font-display text-2xl font-black sm:text-3xl">Produce the remaining 25 tracks without reopening the voice decision.</h2>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Every expansion track uses the exact approved v3 provider prompt, Gemini 3.1 Flash TTS, Sulafat, locked canonical transcript, private Greene storage, checksum/provenance evidence, and no cross-model fallback. The factory queues only one track at a time and stops on quota or generation failure.</p>
          </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {quotaBlockedDay ? (
            <button type="button" onClick={() => void retryBlocked()} disabled={queueing || Boolean(activeRenderDay)} className="button-primary disabled:opacity-45">Retry Day {quotaBlockedDay} once</button>
          ) : (
            <button type="button" onClick={() => setAutoRun((value) => !value)} disabled={loading || !summary?.representativeGateUnlocked || expansionReady >= expansionTotal} className="button-primary disabled:opacity-45">{autoRun ? "Pause expansion factory" : expansionReady ? "Resume expansion factory" : "Start 25-track expansion"}</button>
          )}
        ...
        <p className="mt-2 text-xs leading-5 text-slate-500">If the browser sleeps or closes, completed renders remain stored. Reopen this page and press Resume; the factory reads governed state and continues from the first unfinished day.</p>
      </section>
      ...
      <section className="mt-7 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <aside className="rounded-[2rem] border border-white/10 bg-white/[.025] p-5 sm:p-7">
          ...
          <div className="mt-5 space-y-4 text-sm leading-6 text-slate-300">
            ...
            <p><strong className="text-white">Baseline:</strong> Days 1, 8, 15, 22 and 30 human-approved</p>
            <p><strong className="text-white">Fallback:</strong> disabled</p>
            <p><strong className="text-white">Storage:</strong> private Greene-controlled staging</p>
            <p><strong className="text-white">Provider storage:</strong> disabled</p>
            <p><strong className="text-white">Autoplay:</strong> disabled</p>
            <p><strong className="text-white">Master/release:</strong> not automatic</p>
          </div>
        </aside>
      </section>
    </div>
