...
const SUCCESS_PACING_MS = 75_000;
...
export default function AudioExpansionClientV3() {
  ...
  const refresh = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (!payload.representativeGateUnlocked) {
        ...
        setMessage(`The previous track is safely stored. Provider pacing is holding ${Math.max(1, Math.ceil((pacingUntil - Date.now()) / 1000))}s before Day ${payload.nextDay ?? "—"} to reduce free-tier quota collisions.`);
      } else {
  ...
  useEffect(() => {
    ...
    if (pacingUntil && pacingUntil > Date.now()) {
      setMessage(`The last track is stored. Waiting ${pacingSeconds}s before Day ${nextDay} to reduce collisions with Gemini's free-tier request window.`);
      return;
    }
  ...
  }, [autoRun, loading, queueing, activeRenderDay, quotaBlockedDay, cooldownUntil, cooldownSeconds, pacingUntil, pacingSeconds, summary?.representativeGateUnlocked, expansionReady, expansionTotal, nextDay, queue]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-12">
      <section className="rounded-[2rem] border border-[#49d8c2]/25 bg-[#49d8c2]/[.05] p-5 sm:p-7">
        <p className="text-xs font-black uppercase tracking-[.2em] text-[#76ead6]">25-track expansion factory · quota-aware v3</p>
        <div className="mt-3 grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <h2 className="font-display text-2xl font-black sm:text-3xl">Keep the approved Z-Girl voice while the factory paces itself around provider limits.</h2>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">The factory renders one track at a time. After every successful render it now spaces the next request by 75 seconds, and a quota hit triggers a separate 70-second cooldown plus one controlled retry. It never switches away from Gemini 3.1 + Sulafat.</p>
          </div>
        ...
        {!quotaBlockedDay && autoRun && pacingSeconds > 0 ? <p className="mt-2 text-sm font-black text-sky-200">Provider-safe pacing: {pacingSeconds}s before the next track</p> : null}
        <p className="mt-2 text-xs leading-5 text-slate-500">Completed tracks are already in private Greene storage. Closing or sleeping the browser cannot erase them. If the browser suspends during pacing or cooldown, reopen this page and press Resume.</p>
      </section>
      ...
      <section className="mt-7 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <aside className="rounded-[2rem] border border-white/10 bg-white/[.025] p-5 sm:p-7">
          ...
          <div className="mt-5 space-y-4 text-sm leading-6 text-slate-300">
            ...
            <p><strong className="text-white">Success pacing:</strong> 75 seconds before the next provider request</p>
            <p><strong className="text-white">Quota cooldown:</strong> 70 seconds + one controlled retry</p>
