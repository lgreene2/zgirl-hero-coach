    ...
    <main className="min-h-screen bg-[#061521] text-white">
      ...
      <section className="border-b border-white/10 bg-[radial-gradient(circle_at_80%_15%,rgba(73,216,194,.16),transparent_34%)]">
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:px-12">
          ...
          <h1 className="mt-2 max-w-5xl font-display text-4xl font-black leading-tight tracking-tight sm:text-6xl">Z-Girl 30-Day Audio Expansion Factory</h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">The five-track representative listening gate is approved and the Gemini API project is on the paid tier. The factory produced the remaining English review candidates with the exact approved Z-Girl voice recipe while retaining checksum/provenance evidence and fail-closed voice continuity controls.</p>
          ...
          <p className="mt-5 max-w-3xl text-sm leading-6 text-slate-400">Generation is complete. Whole-library listening QA is now the active gate; no additional rendering is required unless a specific track is later rejected.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href="/library/30-day/master-review" className="button-primary">Open 30-track master review</Link>
            <Link href="/library/30-day/audio-review" className="button-secondary">Representative listening gate</Link>
          </div>
        </div>
      </section>
