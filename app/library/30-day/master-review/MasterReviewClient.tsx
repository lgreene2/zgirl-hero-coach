...
export default function MasterReviewClient() {
  ...
  const item = useMemo(
    () => HERO_WITHIN_30_DAY.find((entry) => entry.day === activeDay) ?? HERO_WITHIN_30_DAY[0],
    [activeDay],
  );
  const transcript = useMemo(() => getHeroWithin30DayTranscript(item), [item]);
  const day = summary?.days?.find((entry) => entry.day === activeDay);
  const currentDecision = decision(day);
  const heard = listened.includes(activeDay) || currentDecision === "APPROVED";

  useEffect(() => {
    try {
      const storedNotes = localStorage.getItem(NOTES_KEY);
      const storedListened = localStorage.getItem(LISTENED_KEY);
      if (storedNotes) setNotes(JSON.parse(storedNotes) as Record<number, string>);
      if (storedListened) setListened(JSON.parse(storedListened) as number[]);
    } catch {
      // Local notes/listening markers are convenience only.
    }
  }, []);
  ...
  useEffect(() => {
    try { localStorage.setItem(LISTENED_KEY, JSON.stringify(listened)); } catch {}
  }, [listened]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setSummary(payload);
      if ((payload.masterCount ?? 0) === 30) {
        setMessage("30/30 governed master records exist. Public release remains a separate gate.");
      } else if (payload.masterPromotionUnlocked) {
        setMessage("All technical, human-listening, and rights gates are complete. Master promotion is available.");
      } else {
        setMessage(`${payload.technicalVerified ?? 0}/30 technical checks verified · ${payload.humanApproved ?? 0}/30 human approvals · ${payload.rightsCleared ?? 0}/30 rights cleared.`);
      }
    } catch (error) {
  ...
  const markListened = () => {
    setListened((current) => current.includes(activeDay) ? current : [...current, activeDay]);
  };
  ...
  const review = async (nextDecision: "APPROVED" | "REJECTED") => {
    ...
    const label = nextDecision === "APPROVED" ? "Approve" : "Reject";
    if (!window.confirm(`${label} Day ${activeDay}? This records human listening QA for the exact stored checksum. It does not publish or create a master.`)) return;
    ...
    try {
      ...
      setMessage(`Day ${activeDay} ${nextDecision === "APPROVED" ? "approved" : "rejected"}; the decision is bound to its stored SHA-256.`);

      if (nextDecision === "APPROVED") {
        const pending = payload.summary?.days?.find((entry) => entry.day > activeDay && !entry.humanDecision)
          ?? payload.summary?.days?.find((entry) => !entry.humanDecision);
  ...
  const promote = async () => {
    ...
    try {
      ...
      if (payload.summary) setSummary(payload.summary);
      setMessage(`${payload.promotedCount ?? 30}/30 governed master records created. Public release remains locked.`);
    } catch (error) {
  ...
  const days = summary?.days ?? Array.from({ length: 30 }, (_, index) => ({ day: index + 1 } as DayQA));
    ...
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-12">
      <section className="rounded-[2rem] border border-[#49d8c2]/25 bg-[#49d8c2]/[.05] p-5 sm:p-7">
        ...
        <div className="mt-3 grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Playback uses the stored candidate and makes no Gemini request. Human listening approval, rights clearance, master promotion, and public release remain separate governance decisions.</p>
          </div>
        ...
        <button type="button" onClick={() => void refresh()} disabled={loading} className="button-secondary mt-4 disabled:opacity-40">{loading ? "Refreshing…" : "Refresh governed state"}</button>
      </section>
      ...
      <section className="mt-7 grid gap-7 lg:grid-cols-[1.35fr_.65fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/[.025] p-5 sm:p-7">
          <p className="text-xs font-black uppercase tracking-[.2em] text-[#76ead6]">Day {activeDay} · {item.theme}</p>
          ...
          <div className="mt-6 rounded-3xl border border-white/10 bg-[#061521]/70 p-4">
            ...
            <p className="mt-2 break-all text-sm text-slate-300">SHA-256: {day?.checksum ?? "—"}</p>
            ...
            <audio key={activeDay} ref={audioRef} controls preload="metadata" className="mt-4 w-full" src={`${API}?day=${activeDay}&audio=1`} onPlay={markListened} />
          ...
          <textarea
            ...
            onChange={(event) => setNotes((current) => ({ ...current, [activeDay]: event.target.value }))}
            rows={4}
            className="mt-2 w-full rounded-2xl border border-white/10 bg-[#061521] p-4 text-white outline-none focus:border-[#49d8c2]"
            placeholder="Voice continuity, pacing, pronunciation, noise, clipping, timing, or other QA notes…"
          />
          <div className="mt-4 flex flex-wrap gap-3">
            <button type="button" disabled={saving || !day?.technicalVerified || !heard} onClick={() => void review("APPROVED")} className="button-primary disabled:opacity-40">{saving ? "Recording…" : currentDecision === "APPROVED" ? "Reconfirm approval" : "Approve and next"}</button>
        ...
        <aside className="space-y-7">
          ...
          <div className="rounded-[2rem] border border-amber-300/25 bg-amber-300/[.04] p-5 sm:p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-300">Requires 30/30 technical validations, 30/30 checksum-bound human approvals, and 30/30 rights clearances. This listening screen cannot self-clear rights.</p>
            ...
            <p className="mt-3 text-xs leading-5 text-slate-500">Promotion creates governed master records only. It does not publish the library, activate subscriptions, or change the public Z-Girl site.</p>
          </div>
        </aside>
      </section>
    </div>
...
function Metric({ value, label }: { value: number; label: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#061521]/70 p-4">
      <p className="text-2xl font-black">{value}</p>
      <p className="mt-1 text-[10px] font-black uppercase tracking-[.1em] text-slate-500">{label}</p>
    </div>
  );
}
