...
export default function AudioReviewClientV6() {
  ...
  const refresh = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (count === 5) setMessage("All five representative candidates are stored. Human listening remains required before mastering or release.");
      ...
      else if (payload.quotaBlockedDay) setMessage(`Day ${payload.quotaBlockedDay} hit the primary-model quota. The worker is now cross-model capable: one deliberate retry can try the fallback TTS model instead of repeating the same failing route.`);
  ...
  const queue = useCallback(async (action: "prepare-next" | "retry-blocked") => {
    ...
    try {
      ...
      else if (payload.day) setMessage(`Day ${payload.day} is queued. The worker can use the fallback TTS model if the primary route remains quota-blocked.`);
      await refresh(true);
    } catch (error) {
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-2">
        <section className="rounded-[2rem] border border-white/10 bg-[#0b2030]/80 p-5 sm:p-7">
          ...
          <div className="mt-6 rounded-3xl border border-white/10 bg-[#061521]/60 p-5">
            ...
            <span className="ml-3 text-xs font-bold text-slate-500">Not a master</span>
            {ready ? (
              <>
