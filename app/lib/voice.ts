...
const NATURAL_HINTS = ["natural", "neural", "premium", "enhanced", "online", "wavenet"];

const FEMININE_HINTS = [
  "female", "woman", "girl", "samantha", "victoria", "karen", "tessa", "serena", "zira", "aria", "jenny",
  "ava", "emma", "olivia", "susan", "hazel", "sonia", "michelle", "helena", "elvira", "dalia", "paloma",
  "monica", "paulina", "sabina", "amelie", "amélie", "hortense", "denise", "audrey", "marie", "francisca",
  "fernanda", "vitoria", "vitória", "katja", "hedda", "amala", "anna", "petra", "allison", "fiona", "kathy",
  "moira", "nicky", "vicki", "zoe", "google uk english female",
];

const MASCULINE_HINTS = [" male", "david", "mark", "george", "daniel", "ralph", "fred", "thomas", "guy"];

// Web Speech exposes all device voices, including novelty/character/compact
// voices that can be distracting or inappropriate for institutional guidance.
const DISTRACTING_NARRATION_HINTS = [
  "albert", "bad news", "bahh", "bells", "boing", "bubbles", "cellos", "good news", "jester", "organ",
  "superstar", "trinoids", "whisper", "wobble", "zarvox", "eddy", "flo", "grandma", "grandpa", "reed",
  "rocko", "sandy", "shelley", "compact", "espeak", "festival",
];

const PROFESSIONAL_NARRATION_HINTS = [
  "siri", "microsoft", "google us english", "google uk english", ...NATURAL_HINTS, ...FEMININE_HINTS,
];
...
export function voiceMatchesLanguage(voice: SpeechSynthesisVoice, lang: string): boolean {
  const target = languageBase(lang);
  return Boolean(target && languageBase(voice.lang) === target);
}

export function isSuitableNarrationVoice(voice: SpeechSynthesisVoice, lang = "en-US"): boolean {
...
export function rankVoices(voices: SpeechSynthesisVoice[], lang: string): SpeechSynthesisVoice[] {
  ...
  return [...safeVoices].sort((a, b) => {
    const difference = score(b) - score(a);
    return difference || a.name.localeCompare(b.name);
  });
}

export function curateNarrationVoices(voices: SpeechSynthesisVoice[], lang = "en-US", limit = 6): SpeechSynthesisVoice[] {
...
export function pickVoice(
  ...
  if (preferredName) {
    ...
    const exactName = (exact?.name || "").toLowerCase();
    if (exact && !DISTRACTING_NARRATION_HINTS.some((hint) => exactName.includes(hint))) return exact;
  }

  const inLang = lang ? voices.filter((voice) => voiceMatchesLanguage(voice, lang)) : voices.slice();
  if (!inLang.length) return null;
  const ranked = lang ? rankVoices(inLang, lang) : inLang;
  if (preferFemale) return ranked[0] || null;
  return ranked.find((voice) => voice.default) || ranked[0] || null;
}
