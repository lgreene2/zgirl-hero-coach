...
import { clearCredentialSession, credentialSessionToken, setCredentialSession } from "@/lib/credentials/session";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (newAccessCode.length < 24 || newAccessCode.length > 200) {
      return Response.json({ ok: false, error: "access_code_too_short" }, { status: 400 });
    }

    const result = await credentialRpc<RotateResult>("zgirl_credential_rotate_access", {
