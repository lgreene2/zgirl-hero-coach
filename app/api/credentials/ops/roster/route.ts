...
export async function GET() {
  ...
  try {
    ...
    const rows = credentials.map((credential) => [
      ...
      credential.training_version,
      credential.public_verification_enabled ? "yes" : "no",
      credential.scope,
      `https://zgirlinitiative.org/credentials/record/${credential.credential_id}`,
    ]);
    ...
    return new Response(csv, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="zgirl-credential-roster-${stamp}.csv"`,
