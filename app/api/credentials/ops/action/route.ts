...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "set_requirement") {
      ...
      const requirementId = await credentialRpc<string>("zgirl_credential_set_requirement", {
    ...
    if (action === "issue") {
      ...
      const credential = await credentialRpc<Record<string, unknown>>("zgirl_credential_issue", {
        p_session_token: token,
        p_candidate_id: candidateId,
        p_credential_level: credentialLevel,
        p_scope: scope,
    ...
    if (action === "change_status") {
      ...
      await credentialRpc<boolean>("zgirl_credential_change_status", {
        p_session_token: token,
        p_credential_id: credentialId,
        p_status: status,
        p_reason_category: reason,
        p_public_verification_enabled: publicVerificationEnabled,
      });
    ...
    if (action === "renew") {
      ...
      await credentialRpc<boolean>("zgirl_credential_renew", {
        p_session_token: token,
        p_credential_id: credentialId,
