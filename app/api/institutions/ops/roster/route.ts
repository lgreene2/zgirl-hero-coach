...
type Dashboard = { licenses?: Array<Record<string, unknown>>; allocations?: Array<Record<string, unknown>>; institutions?: Array<Record<string, unknown>>; };
...
export async function GET(request: Request) {
  try {
    ...
    const body = rows.map((item) => { const license = byId.get(String(item.license_id || "")) || {}; return [license.institution_name,license.license_code,license.license_type,license.status,item.seat_role,item.status,item.site_name,item.full_name,item.credential_code,item.credential_level].map(csv).join(","); });
