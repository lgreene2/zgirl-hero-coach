...
export async function GET() {
  try {
    const {token}=await requireOperatorCapability("workflow.read");
    const dashboard = await credentialRpc<Record<string, unknown>>("zgirl_institution_workflow_dashboard", { p_session_token: token });
