...
const WORKFLOW_TYPES = new Set(["renewal", "expansion", "change_order", "train_the_trainer_addendum"]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "save_agreement") {
      ...
      const effectiveDate = optionalDate(body.effectiveDate);
      ...
      const id = await credentialRpc<string>("zgirl_institution_save_agreement", {
    ...
    if (action === "create_workflow") {
      ...
      const agreementId = typeof body.agreementId === "string" && UUID.test(body.agreementId) ? body.agreementId : null;
      const requestedEffectiveDate = optionalDate(body.requestedEffectiveDate);
      const requestedExpiresAt = optionalDate(body.requestedExpiresAt);
      const requestedSeatLimit = optionalInt(body.requestedSeatLimit);
      const requestedSiteLimit = optionalInt(body.requestedSiteLimit);
      const requestedTrainerLimit = optionalInt(body.requestedTrainerLimit);
      const requestedProfiles = strings(body.requestedProfiles);
      const requestedLevels = strings(body.requestedLevels);
      ...
      const id = await credentialRpc<string>("zgirl_institution_create_workflow", {
        p_session_token: token,
        p_workflow_id: workflowId,
        p_license_id: licenseId,
        p_workflow_type: workflowType,
        p_agreement_id: agreementId,
        p_requested_effective_date: requestedEffectiveDate,
        p_requested_expires_at: requestedExpiresAt,
        p_requested_seat_limit: requestedSeatLimit,
        p_requested_site_limit: requestedSiteLimit,
        p_requested_trainer_limit: requestedTrainerLimit,
        p_requested_profiles: requestedProfiles.length ? requestedProfiles : null,
        p_requested_levels: requestedLevels.length ? requestedLevels : null,
    ...
    if (action === "link_agreement") {
      ...
      await credentialRpc<boolean>("zgirl_institution_link_workflow_agreement", { p_session_token: token, p_workflow_id: workflowId, p_agreement_id: agreementId });
      return Response.json({ ok: true });
    }

    if (action === "build_evidence") {
      const workflowId = typeof body.workflowId === "string" ? body.workflowId : "";
      if (!UUID.test(workflowId)) return bad("workflow_not_found");
      const packetId = await credentialRpc<string>("zgirl_institution_build_evidence_packet", { p_session_token: token, p_workflow_id: workflowId });
    ...
    if (action === "set_gate") {
      ...
      const decisionReference = typeof body.decisionReference === "string" ? body.decisionReference.trim() : "";
      ...
      await credentialRpc<boolean>("zgirl_institution_set_approval_gate", { p_session_token: token, p_workflow_id: workflowId, p_gate_key: gateKey, p_status: status, p_decided_by: decidedBy, p_decision_reference: decisionReference });
      return Response.json({ ok: true });
    }

    if (action === "finalize_workflow") {
      const workflowId = typeof body.workflowId === "string" ? body.workflowId : "";
      const implementationOwner = typeof body.implementationOwner === "string" ? body.implementationOwner.trim() : "";
      const handoffReference = typeof body.handoffReference === "string" ? body.handoffReference.trim() : "";
      ...
      const handoffId = await credentialRpc<string>("zgirl_institution_finalize_workflow", { p_session_token: token, p_workflow_id: workflowId, p_implementation_owner: implementationOwner, p_handoff_reference: handoffReference });
    ...
    if (action === "release_handoff") {
      ...
      const releaseReference = typeof body.releaseReference === "string" ? body.releaseReference.trim() : "";
      if (!UUID.test(handoffId) || releaseReference.length < 3 || releaseReference.length > 180) return bad("invalid_handoff");
      await credentialRpc<boolean>("zgirl_institution_release_handoff", { p_session_token: token, p_handoff_id: handoffId, p_release_reference: releaseReference });
      return Response.json({ ok: true });
    }

    if (action === "run_automation") {
      const automation = await credentialRpc<Record<string, unknown>>("zgirl_institution_workflow_run_automation", { p_session_token: token });
