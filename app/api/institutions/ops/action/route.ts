...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "save_institution") {
      ...
      const type = typeof body.institutionType === "string" ? body.institutionType : "";
    ...
    if (action === "save_site") {
      const siteId = typeof body.siteId === "string" && UUID.test(body.siteId) ? body.siteId : null;
      const institutionId = typeof body.institutionId === "string" ? body.institutionId : "";
      const name = typeof body.name === "string" ? body.name.trim() : "";
      const siteType = typeof body.siteType === "string" ? body.siteType : "";
      const status = typeof body.status === "string" ? body.status : "";
    ...
    if (action === "save_license") {
      ...
      const institutionId = typeof body.institutionId === "string" ? body.institutionId : "";
      const licenseType = typeof body.licenseType === "string" ? body.licenseType : "";
      const status = typeof body.status === "string" ? body.status : "";
      ...
      const seatLimit = Number(body.seatLimit); const siteLimit = Number(body.siteLimit); const trainerLimit = Number(body.trainerLimit);
      const profiles = strings(body.allowedProfiles); const levels = strings(body.allowedLevels);
      const agreementStatus = typeof body.agreementStatus === "string" ? body.agreementStatus : "draft";
    ...
    if (action === "allocate_seat") {
      const licenseId = typeof body.licenseId === "string" ? body.licenseId : ""; const candidateId = typeof body.candidateId === "string" ? body.candidateId : "";
      const siteId = typeof body.siteId === "string" && UUID.test(body.siteId) ? body.siteId : null; const seatRole = typeof body.seatRole === "string" ? body.seatRole : "";
