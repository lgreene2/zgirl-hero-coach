...
export async function POST(request: Request) {
  try {
    ...
    if (action === "create_prospect_opportunity") {
      const {token}=await requireOperatorCapability("pipeline.write");
      const name = text(body.institutionName, 180); const institutionType = text(body.institutionType, 40); const contactName = text(body.contactName, 160); const contactEmail = text(body.contactEmail, 254).toLowerCase();
      ...
      const institutionId = await credentialRpc<string>("zgirl_institution_save", {p_session_token: token,p_institution_id: null,p_name: name,p_institution_type: institutionType,p_status: "prospect",p_contact_name: contactName,p_contact_email: contactEmail});
      const opportunityType = text(body.opportunityType, 40); const contractPath = text(body.contractPath, 50);
      if (!OPPORTUNITY_TYPES.has(opportunityType) || contractPath !== "initial_contract") return bad("invalid_opportunity_type");
      const opportunityId = await credentialRpc<string>("zgirl_partner_save_opportunity", {p_session_token: token,p_opportunity_id: null,p_institution_id: institutionId,p_existing_license_id: null,p_opportunity_type: opportunityType,p_contract_path: contractPath,p_stage: "identified",p_priority: PRIORITIES.has(text(body.priority, 20)) ? text(body.priority, 20) : "normal",p_owner_name: text(body.ownerName, 120),p_source: text(body.source, 160),p_strategic_need: text(body.strategicNeed),p_use_case: text(body.useCase),p_estimated_value_cents: cents(body.estimatedValueCents),p_currency: "USD",p_probability_percent: optionalInt(body.probabilityPercent) ?? 10,p_target_decision_date: optionalDate(body.targetDecisionDate),p_target_start_date: optionalDate(body.targetStartDate),p_closed_reason: ""});
      if (contactName) await credentialRpc<string>("zgirl_partner_save_contact", {p_session_token: token,p_contact_id: null,p_opportunity_id: opportunityId,p_full_name: contactName,p_role_title: text(body.contactRoleTitle, 160),p_email: contactEmail,p_phone: text(body.contactPhone, 60),p_decision_role: "champion",p_status: "active"});
      return Response.json({ ok: true, institutionId, opportunityId });
    }

    if (action === "save_opportunity") {
      const opportunityId = optionalUuid(body.opportunityId); const institutionId = requiredUuid(body.institutionId); const existingLicenseId = optionalUuid(body.existingLicenseId);
      const opportunityType = text(body.opportunityType, 40); const contractPath = text(body.contractPath, 50); const stage = text(body.stage, 40); const priority = text(body.priority, 20);
      const estimatedValueCents = cents(body.estimatedValueCents); const probability = optionalInt(body.probabilityPercent); const targetDecisionDate = optionalDate(body.targetDecisionDate); const targetStartDate = optionalDate(body.targetStartDate);
      ...
      const {token}=await requireOperatorCapability("pipeline.write",institutionId);
      const id = await credentialRpc<string>("zgirl_partner_save_opportunity", {p_session_token: token,p_opportunity_id: opportunityId,p_institution_id: institutionId,p_existing_license_id: existingLicenseId,p_opportunity_type: opportunityType,p_contract_path: contractPath,p_stage: stage,p_priority: priority,p_owner_name: text(body.ownerName, 120),p_source: text(body.source, 160),p_strategic_need: text(body.strategicNeed),p_use_case: text(body.useCase),p_estimated_value_cents: estimatedValueCents,p_currency: "USD",p_probability_percent: probability,p_target_decision_date: targetDecisionDate,p_target_start_date: targetStartDate,p_closed_reason: text(body.closedReason, 600)});
      return Response.json({ ok: true, opportunityId: id });
    }

    if (action === "save_contact") {
      const opportunityId = requiredUuid(body.opportunityId); const contactId = optionalUuid(body.contactId); const fullName = text(body.fullName, 160); const email = text(body.email, 254).toLowerCase(); const decisionRole = text(body.decisionRole, 30); const status = text(body.status, 20);
      ...
      const {token}=await requireEntityCapability("pipeline.write","opportunity",opportunityId);
    ...
    if (action === "log_activity") {
      const opportunityId = requiredUuid(body.opportunityId); const activityType = text(body.activityType, 30); const direction = text(body.direction, 20); const summary = text(body.summary, 1600); const occurredAt = optionalTimestamp(body.occurredAt);
    ...
    if (action === "save_proposal") {
      const opportunityId = requiredUuid(body.opportunityId); const proposalId = optionalUuid(body.proposalId); const proposalType = text(body.proposalType, 40); const status = text(body.status, 30); const version = optionalInt(body.version); const proposedValueCents = cents(body.proposedValueCents); const sentAt = optionalTimestamp(body.sentAt); const expiresAt = optionalDate(body.expiresAt);
    ...
    if (action === "complete_followup") {
      ...
      const {token}=await requireEntityCapability("pipeline.write","followup",followupId);
    ...
    if (action === "handoff_to_workflow") {
      const opportunityId = requiredUuid(body.opportunityId); const proposalId = requiredUuid(body.proposalId); const effectiveDate = optionalDate(body.effectiveDate); const expiresAt = optionalDate(body.expiresAt); const seatLimit = optionalInt(body.seatLimit); const siteLimit = optionalInt(body.siteLimit); const trainerLimit = optionalInt(body.trainerLimit); const profiles = strings(body.profiles); const levels = strings(body.levels);
      ...
      const {token}=await requireEntityCapability("pipeline.handoff","opportunity",opportunityId);
