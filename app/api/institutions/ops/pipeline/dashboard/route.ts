...
export async function GET() {
  try {
    const {token}=await requireOperatorCapability("pipeline.read");
    const dashboard = await credentialRpc<Record<string, unknown>>("zgirl_partner_pipeline_dashboard", { p_session_token: token });
