...
const offers = [
  { title: "Athlete Reflection Starter Pack", audience: "Individual and family", copy: "Printable and digital prompts for focus, mistakes, confidence, teamwork, postgame reflection, and parent conversation.", href: "/resources/Hero_Within_Athlete_Reflection_Starter_Pack.pdf", cta: "Download PDF", external: true },
  { title: "Coach Toolkit", audience: "Coach and team leader", copy: "A four-week implementation plan with five-minute reflections, scripts, culture prompts, parent communication, and inclusive participation guidance.", href: "/athletes/coach", cta: "Explore toolkit", external: false },
  { title: "Team Pilot", audience: "School, team, league, or organization", copy: "Coach orientation, digital samples, team worksheets, parent information, implementation support, and completion feedback.", href: "/athletes/pilot", cta: "View pilot offer", external: false },
];
          ...
          <aside className="overflow-hidden rounded-[2.1rem] border border-[#49d8c2]/20 bg-[#0a2030]/95 shadow-2xl shadow-black/30">
            ...
            <div className="border-t border-white/10 p-5 sm:p-6">
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-5 lg:grid-cols-1 xl:grid-cols-5">
                {["Pause", "Name It", "Learn", "Choose", "Return"].map((title, index) => <div key={title} className="rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 text-center"><span className="text-xs font-black text-[#76ead6]">{index + 1}</span><p className="mt-1 text-xs font-bold text-slate-200">{title}</p></div>)}
              </div>
            </div>
          </aside>
        </div>
