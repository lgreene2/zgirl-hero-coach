...
- `/coach` — optional AI-guided reflection with youth, adult, and supporter editions; English (US) includes a listening-gated AI-generated voice candidate with device fallback
...
The v3.14.1 Natural AI Voice Candidate replaces the misleading browser-only natural-voice label in the live Coach with a server-side, AI-generated English (US) voice candidate. It sends only the selected visible Z-Girl reply, disables provider-side Interaction storage, never autoplays, visibly discloses the AI voice data flow, and retains a clearly labeled device fallback. Production promotion remains blocked until product-owner listening approval on iPhone and desktop.
