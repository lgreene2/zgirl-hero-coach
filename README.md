...
The v3.14.1 Natural AI Voice release replaces the misleading browser-only natural-voice label in the live Coach with a server-side, AI-generated English (US) voice. It sends only the selected visible Z-Girl reply, disables provider-side Interaction storage, never autoplays or plays a startup/voice-start chime, visibly discloses the AI voice data flow, deduplicates in-flight Speak requests, and does not silently substitute a robotic device voice. Product-owner listening approval was recorded on August 24, 2026 after successful iPhone review.
