...
function validateRecord(record) {
  ...
  for (const locale of EXPECTED_LOCALES) {
    const credential = localeMap.get(locale);
  ...
  if (
    ...
    throw new Error(
      "Activation record is missing ZGIRL_ASSET_GATEWAY_BEARER_TOKEN.",
    );
  }
  if (
  ...
  return { localeMap, publicVars, privateVars };
}
...
if (-not [string]::IsNullOrWhiteSpace($GatewayDeploymentUrl)) {
  Write-Host "Redeploying the private gateway..."
  Invoke-Vercel redeploy $GatewayDeploymentUrl --yes --scope $Scope
}
if (-not [string]::IsNullOrWhiteSpace($PublicDeploymentUrl)) {
  Write-Host "Redeploying the public Z-Girl project..."
  Invoke-Vercel redeploy $PublicDeploymentUrl --yes --scope $Scope
}
...
& node (Join-Path $BundleRoot "verify-review-activation.mjs") `
  --record (Join-Path $BundleRoot "activation-record.json") `
  --public-url $PublicUrl

if ($LASTEXITCODE -ne 0) {
  throw "Z-Girl reviewer activation verification failed."
}
...
function readmeText(gatewayBaseUrl) {
  return `Z-GIRL v2.2.3 CONFIDENTIAL ACTIVATION BUNDLE
...
REDEPLOYMENT
Environment changes apply only to new deployments. Redeploy the private gateway and then the public zgirl-hero-coach project. The PowerShell script can also redeploy when deployment URLs are supplied through -GatewayDeploymentUrl and -PublicDeploymentUrl.
...
The verifier tests:
- four reviewer logins;
- all 56 proxied audio tracks;
- byte-range audio behavior;
- private/no-store responses;
- invalid-code rejection;
- unauthenticated blocking;
- cross-language session isolation;
- protected review-page access;
- logout cookie clearing.

REVIEWER HANDOFF
reviewer-credentials.csv contains one plaintext code per language. Send each reviewer only the row assigned to that reviewer. Do not send the full file.
...
await mkdir(outDir, { recursive: false, mode: 0o700 }).catch((error) => {
  if (error?.code === "EEXIST") {
    throw new Error(
      `Output directory already exists: ${outDir}. Choose a new private directory.`,
    );
  }
  throw error;
});
...
await Promise.all([
  ...
  secureWrite(
    path.join(outDir, "private-gateway.env"),
    envText(privateEnvironment),
  ),
  secureWrite(
    path.join(outDir, "public-zgirl.env"),
    envText(publicEnvironment),
  ),
  secureWrite(
  ...
  secureWrite(
    path.join(outDir, "apply-vercel-env.ps1"),
    powershellScript(),
  ),
  secureWrite(
  ...
  secureWrite(
    path.join(outDir, "verify-review-activation.mjs"),
    verifierSource,
  ),
...
console.log(
  "No reviewer code, bearer token, session secret, or hash map was displayed.",
);
